...
const TopLight = (function () {
  ...
  function getDashboardData_(data, user, dbId) {
    const kpiAuthorized = !!(user && (user.isSuperAdmin || (user.authorizedPages && user.authorizedPages['tl_dashboard'] && user.authorizedPages['tl_dashboard'].indexOf('write') !== -1)));
    return {
      status: 'success',
  ...
  function getKpiData_(data, user, dbId) {
    const kpiAuthorized = !!(user && (user.isSuperAdmin || unifiedCheck_(user, '8df5c89a117fe9a5', 'tl_kpi', 'write')));
    return {
      status: 'success',
      kpi_authorized: kpiAuthorized,
      kpis: kpiAuthorized ? dashboardKpis_(dbId) : null
  ...
  function currencyOptions_() {
    return cachedMap_('tl_curr_opts', 300, function () {
      try {
  ...
  function bustTopLightCaches_(dbId, type) {
    try {
      ...
      if (!type || type === 'parties') keys.push('tl_cust_sales_opts_' + dbId);
      if (!type || type === 'products') { keys.push('tl_qty_map_' + dbId, 'tl_price_map_' + dbId, 'tl_cat_opts_' + dbId); }
